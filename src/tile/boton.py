...
class Boton(ObjetoEvento):
    def __init__(self, color: str, interaccion_pulsar: Callable[..., None], interaccion_soltar: Callable[..., None],
                 scale: float, center_x: float = 0, center_y: float = 0, angle: float = 0) -> None:
        path_sin_pulsar = Path("assets") / "images" / f"boton_{color}_sin_pulsar.png"
        path_pulsado = Path("assets") / "images" / f"boton_{color}_pulsado.png"

        super().__init__([interaccion_pulsar], [interaccion_soltar], path_sin_pulsar, scale, center_x, center_y, angle)
